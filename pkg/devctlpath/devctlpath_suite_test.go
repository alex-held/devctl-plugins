...
var _ = Describe("Pather", func() {
	...
	Context("ConfigFilers", func() {
		...
		Describe("ConfigFilePath()", func() {
			BeforeEach(func() {
				expected = path.Join(osDefault, prefix, "config.yaml")
			})

			It("returns the default config file path", func() {
				Expect(sut.ConfigFilePath()).Should(Equal(expected))
			})
		})

		Describe("PluginConfigFilePath()", func() {
			BeforeEach(func() {
				expected = path.Join(osDefault, prefix, "plugins.toml")
			})

			It("returns the default plugin config file path", func() {
				Expect(sut.PluginConfigFilePath()).Should(Equal(expected))
			})
		})
	})
